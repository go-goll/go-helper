...
func GinLogMiddleware(logger zerolog.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		startT := time.Now()
		id := c.Request.Header.Get(loghelper.ReqIdHeaderName)
		if id == "" {
			id = loghelper.GenerateReqId()
		}
		...
		l := logger.With().Str(loghelper.ReqIdContextName, id).Logger()
		...
		ctx = context.WithValue(ctx, loghelper.ReqIdContextName, id)
		c.Request = c.Request.WithContext(ctx)

		// print req logmiddleware
		body := reqBody(c)
		reqInfo := reqJson(c)
		event := l.Debug().Str("type", "REQUEST").RawJSON("reqInfo", reqInfo).RawJSON("body", body)
		if PrintHeaders {
		...
		c.Writer.Header().Set(loghelper.ReqIdHeaderName, id)
